...
import com.mrx.indoorservice.api.IndoorService.getInstance
...
class VuforiaActivity : AppCompatActivity(), GLSurfaceView.Renderer, SurfaceHolder.Callback {
    ...
    private var indoorService: IndoorService? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        indoorService = getInstance(this)
        ...
        cameraLayout.addView(
            mGLView, ViewGroup.LayoutParams(
        ...
        findViewById<LinearLayout>(R.id.fuckingLayout).visibility = View.GONE
        findViewById<LinearLayout>(R.id.vuforiaLayout).visibility = View.VISIBLE
    ...
    override fun onRequestPermissionsResult(
        ...
        if (permissions.isEmpty()) {
            ...
            Toast.makeText(
                this,
                "The permission request was cancelled. You must grant Camera permission to access AR features of this application.",
                Toast.LENGTH_LONG
            ).show()
            finish()
            return
        ...
        if (runtimePermissionsGranted()) {
            ...
            Toast.makeText(
                this,
                "You must grant Camera permission to access AR features of this application.",
                Toast.LENGTH_LONG
            ).show()
            finish()
        }
    ...
    override fun onDrawFrame(unused: GL10) {
        if (mVuforiaStarted) {
            ...
            val vecX = searchingX - personX
            val vecY = searchingY - personY
            angle = acos(vecY / sqrt(vecX * vecX + vecY * vecY)) * 180 / PI

            /*angle = acos(
                (searchingX * personX + searchingY * personY) /
                        (sqrt(personX * personX + personY * personY) * sqrt(searchingX * searchingX + searchingY * searchingY))
            )*/
            if (personX > searchingX) angle = 360 - angle

            arrowImage.rotation = ((indoorService!!.AzimuthManager.getAzimuthViewModel().value ?: 0F) + angle.toFloat())%360 + 90F
        }
    }
